...
function ModalFormProductos({ setOpen, method, product }) {
  ...
  }, [method, product, setValue, navigate]);
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      if (method === 'register') {
        ...
        const codigoExistente = productos.find((producto) => producto.codigo === data.codigo);
        if (codigoExistente) {
        ...
        // Crear producto
        const response = await createProducto(data, selectedCategory.id, file);
        if (response === 201) {
    ...
    <div className="fixed inset-0 flex justify-center items-center bg-black bg-opacity-50 z-50">
      <form onSubmit={onSubmit} className="flex flex-col overflow-y-auto bg-white gap-5 w-[500px] rounded-t-lg">
        <div className='relative'>
          {
            method === 'register' ? <></> : (
              <img src={product.image} className='w-full h-52 object-cover rounded-md mb-7' alt='Product' />
            )
          }
